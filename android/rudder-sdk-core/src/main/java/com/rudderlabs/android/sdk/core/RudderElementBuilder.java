...
public class RudderElementBuilder {
    ...
    public RudderElement build() {
        ...
        if (this.userId != null) event.setUserId(userId);
        if (this.eventName != null) event.setEventName(eventName);
        if (this.property != null) event.setProperty(property);
        if (this.userProperty != null) event.setUserProperty(userProperty);
        return event;
    }
}
